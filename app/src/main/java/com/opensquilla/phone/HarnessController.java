...
/**
 * 核心控制器：分步安装（rootfs / 基础工具 / Node / opensquilla，每步可单独
...
public class HarnessController {
    ...
    private static final String PREFS = "opensquilla";
    ...
    private String errMsg(String prefix, Throwable e) {
        ...
        return prefix + "（OpenSquilla v" + v + "）" + describe(e);
    }
    ...
    public static String stepName(int step) {
        switch (step) {
            ...
            case STEP_HARNESS: return "⑤ opensquilla";
    ...
    public String getPort() {
        // 兜底校验：空/非数字/越界全部回退默认 18791（否则 --port 后是空串导致启动失败）
        String p = prefs.getString("port", "18791");
        if (p == null) return "18791";
        String t = p.trim();
        if (t.isEmpty()) return "18791";
        try {
            int n = Integer.parseInt(t);
            if (n < 1 || n > 65535) return "18791";
    ...
    public String getWorkdir() { return prefs.getString("workdir", "opensquilla"); }
    ...
    /** 实际工作目录自愈：prefs 指定名不存在时，扫描 rootfs /root 下含 apps/cli/gateway 的目录并回写 */
    public String detectWorkdir() {
        ...
        try {
            ...
            if (dirs != null) for (java.io.File d : dirs) {
                if (new java.io.File(d, "apps/cli/gateway").isFile()
                        || new java.io.File(d, "gateway").isFile()) {
    ...
    private boolean workdirExists(String wd) {
        try {
            return new java.io.File(proot.getRootfsDir(), "root/" + wd + "/apps/cli/gateway").isFile()
                    || new java.io.File(proot.getRootfsDir(), "root/" + wd + "/gateway").isFile();
        } catch (Exception e) {
            return false;
        }
    }
    ...
    /** 是否已安装 opensquilla（跟随自定义工作区路径；RC6 模式检查 dsh 命令） */
    ...
    /** ④ opensquilla：预构建包 或 直连源码构建（全局进度 90~100） */
    ...
    public boolean useRc6() {
        return appContext.getSharedPreferences("opensquilla", android.content.Context.MODE_PRIVATE)
    ...
    private void installHarnessFromSource() throws Exception {
        ...
        setProgress("获取 opensquilla 源码", 93);
        runStep("获取 opensquilla 源码", 93,
                "cd /root && " +
                "if [ -d " + wd + " ] && [ -f " + wd + "/package.json ]; then " +
                "echo '源码已存在，跳过克隆（尝试增量更新）'; " +
                "(cd " + wd + " && git pull --ff-only 2>/dev/null || true); " +
                "else rm -rf " + wd + " && ( " +
                "git clone --depth 1 https://github.com/deepseek-ai/opensquilla.git " + wd + " || " +
                "git clone --depth 1 https://gitclone.com/github.com/deepseek-ai/opensquilla.git " + wd + " || " +
                "git clone --depth 1 https://ghfast.top/https://github.com/deepseek-ai/opensquilla.git " + wd + " || " +
                "git clone --depth 1 https://gh-proxy.com/https://github.com/deepseek-ai/opensquilla.git " + wd + " || " +
                "git clone --depth 1 https://ghproxy.net/https://github.com/deepseek-ai/opensquilla.git " + wd + " || " +
                "git clone --depth 1 https://gitcode.com/gh_mirrors/de/opensquilla.git " + wd + " ) || " +
                "(echo 'git 克隆失败，改用源码包下载…'; rm -rf " + wd + " && " +
                "(curl -kfsSL --retry 3 -m 300 https://codeload.github.com/deepseek-ai/opensquilla/tar.gz/refs/heads/main -o dsh-src.tar.gz || " +
                "curl -kfsSL --retry 3 -m 300 https://ghfast.top/https://codeload.github.com/deepseek-ai/opensquilla/tar.gz/refs/heads/main -o dsh-src.tar.gz || " +
                "curl -kfsSL --retry 3 -m 300 https://gh-proxy.com/https://codeload.github.com/deepseek-ai/opensquilla/tar.gz/refs/heads/main -o dsh-src.tar.gz || " +
                "curl -kfsSL --retry 3 -m 300 https://ghproxy.net/https://codeload.github.com/deepseek-ai/opensquilla/tar.gz/refs/heads/main -o dsh-src.tar.gz) && " +
                "tar -xzf dsh-src.tar.gz && mv opensquilla-main " + wd + " && rm -f dsh-src.tar.gz); fi");
        ...
        try {
            runStep("安装 dsh 命令", 99,
                    "ln -sf /root/" + wd + "/apps/cli/gateway /usr/local/bin/dsh && " +
    ...
    public String startWebCommand() {
        // 局域网访问：opensquilla 官方 CLI 默认拒绝 --host 0.0.0.0，
        // 需先打 lan-bind-patch.sh 放行（失败则回落到 127.0.0.1，服务保证能起）。
        boolean lan = appContext.getSharedPreferences("opensquilla", android.content.Context.MODE_PRIVATE)
                .getBoolean("lan_mode", false);
        boolean lanReady = lan && tryEnableLanBind();
        StringBuilder sb = new StringBuilder();
        sb.append("export OpenSquilla_HOME=/root/.dsh && ")
          .append("export DEEPSEEK_API_KEY='").append(effectiveApiKey()).append("' && ")
          .append("export OpenSquilla_PERMISSION_MODE=").append(getPermissionMode()).append(" && ")
          ...
          .append("export OpenSquilla_CONFIRM=1 && ")
          ...
          .append(lanReady ? "echo '[OpenSquilla] 局域网访问(App桥): http://$(hostname -I 2>/dev/null | cut -d' ' -f1):3081' && "
                  : lan ? "echo '[OpenSquilla] 局域网未开启(官方 0.0.0.0 未放行)，仅本机可访问' && " : "")
    ...
    private String depsSelfHeal() {
        ...
        return "node -e \"['@deepseek-ai/dsh-app-boot','@deepseek-ai/dsh-workspace','@deepseek-ai/dsh-session','@deepseek-ai/dsh-base'].forEach(function(m){try{require.resolve(m)}catch(e){process.exit(1)}})\" 2>/dev/null || "
                + "{ echo '[OpenSquilla] 检测到 harness 依赖缺失，正在自动修复…'; "
     ...
     *  自动判断：源码目录存在 → cd + 依赖自愈 + node apps/cli/gateway web；
     *  否则回退全局 dsh web（预构建/目录缺失场景）。含 exec 与日志重定向。 */
    private String runCoreCommand(boolean lanReady) {
        int port = parsePort();
        // 默认端口(18791)不显式传 --port —— 彻底避免 commander 报 'argument missing'；
        // 只有用户自定义端口才追加 --port
        String opts = "";
        if (port != 18791) opts += " --port " + port;
        ...
        return "if [ -d /root/" + wd + " ]; then cd /root/" + wd + "; " + depsSelfHeal()
                + "exec node apps/cli/gateway web" + opts + " > ~/dsh-web.log 2>&1; "
                + "else echo '[OpenSquilla] 源码目录缺失，尝试全局 dsh'; "
                + "if command -v dsh >/dev/null 2>&1 && test -f \"$(command -v dsh)\"; then "
                + "exec dsh web" + opts + " > ~/dsh-web.log 2>&1; "
                + "else echo '[OpenSquilla] 全局 dsh 不可用（悬空链接或未安装），请到分步安装页重装 ⑤ opensquilla'; exit 1; fi; fi";
    }
    ...
    private void writeWatchdogFiles(String restartCmd, int port) {
        try {
            ...
            String restart =
                    "#!/bin/bash\n" +
                    "export OpenSquilla_HOME=/root/.dsh\n" +
                    "export DEEPSEEK_API_KEY='" + effectiveApiKey() + "'\n" +
                    "export OpenSquilla_PERMISSION_MODE=" + getPermissionMode() + "\n" +
                    "export OpenSquilla_CONFIRM=1\n" +
                    "cd /root/" + getWorkdir() + " || exit 1\n" +
                    "mkdir -p /root/.codex/pets /root/.dsh/plugins 2>/dev/null\n" +
                    restartCmd + "\n";
            String watchdog =
                    "#!/bin/bash\n" +
                    "# OpenSquilla 看门狗：WebUI 失联 3 次（约 90 秒）自动重启\n" +
     ...
     * 移除 opensquilla CLI 对 --host 0.0.0.0 的拒绝（底层 webServer 本就支持）。
    ...
    private String startWebTermuxCommand() {
        ...
        sb.append("export PATH=$HOME/dsh-bin:$PATH && ")
          ...
          .append("export DEEPSEEK_API_KEY=\"").append(effectiveApiKey()).append("\" && ")
          .append("export OpenSquilla_PERMISSION_MODE=\"").append(getPermissionMode()).append("\" && ")
          .append("nohup node apps/cli/gateway web > ~/dsh-web.log 2>&1 & echo started");
        return sb.toString();
    }
    ...
    /** 导出已启用插件为 tar.gz（Android Download/OpenSquilla 目录，MediaStore）
    ...
    public String exportPlugins() {
        try {
            ...
            for (String d : PLUGIN_DIRS) {
                ...
                String name = "OpenSquilla-plugins-" + new java.text.SimpleDateFormat("yyyyMMdd-HHmmss", java.util.Locale.US).format(new java.util.Date()) + ".tar.gz";
    ...
    public String fetchMarketIndex() {
        ...
        for (String u : urls) {
            try {
                ...
                conn.setRequestProperty("User-Agent", "OpenSquilla/" + getVersionName());
    ...
    public String removePlugin(String pkg) {
        ...
        try {
            ...
            log.append("[OpenSquilla] 实体清理").append(cleared ? "完成 ✅" : "失败（仍存在）⚠️").append("\n");
            ...
            String py = "python3 - <<'PY'\n" +
                    ...
                    " print('[OpenSquilla] manifest 修改失败:',e); sys.exit(1)\n" +
                    "print('[OpenSquilla] manifest 已移除: " + esc + "')\n" +
                    "PY";
            String r = proot.execAndRead(
                    "( dsh plugin --profile web remove " + esc + " 2>&1 || " +
                    "node apps/cli/gateway plugin --profile web remove " + esc + " 2>&1 || " +
                    "echo '[OpenSquilla] dsh remove 未生效，走 manifest 直改' ) ; " +
                    py + "; echo REMOVE_EXIT=$?");
            log.append(r);
        } catch (Exception e) {
    ...
    private String runPluginInstall(String pkg) {
        try {
            ...
            return proot.execAndRead(
                    ...
                    "printf 'registry=https://registry.npmmirror.com\\n' > /root/.npmrc; " +
                    "( node apps/cli/gateway plugin --profile web add " + pkg + " 2>&1 || dsh plugin --profile web add " + pkg + " 2>&1 ); " +
                    "else echo '[OpenSquilla] 无源码目录，回退全局 dsh'; " +
                    "printf 'registry=https://registry.npmmirror.com\\n' > /root/.npmrc; " +
    ...
    private String copyToDownloads(java.io.File src, String name) {
        ...
        try {
            ...
            cv.put(android.provider.MediaStore.MediaColumns.RELATIVE_PATH, android.os.Environment.DIRECTORY_DOWNLOADS + "/OpenSquilla");
            ...
            if (uri != null) {
                ...
                return android.os.Environment.getExternalStoragePublicDirectory(
                        android.os.Environment.DIRECTORY_DOWNLOADS) + "/OpenSquilla/" + name;
            }
        } catch (Exception ignored) {
        }
        ...
        try {
            if (android.os.Build.VERSION.SDK_INT >= 30 && android.os.Environment.isExternalStorageManager()) {
                java.io.File dir = new java.io.File(android.os.Environment.getExternalStoragePublicDirectory(
                        android.os.Environment.DIRECTORY_DOWNLOADS), "OpenSquilla");
                if (dir.isDirectory() || dir.mkdirs()) {
