...
public class HarnessService extends Service {
    ...
    public static final String ACTION_START = "com.opensquilla.app.START";
    public static final String ACTION_STOP = "com.opensquilla.app.STOP";
    ...
    public void onCreate() {
        ...
        startForeground(NOTIF_ID, buildNotification("OpenSquilla运行中", "Web UI 正在后台保持运行"));
        ...
        taskNotifier.start();
        // 局域网转发桥：开启局域网模式时，App 侧 0.0.0.0:3081 → 127.0.0.1:18791
        ...
        keepAliveThread = new Thread(() -> {
            ...
            while (keepAliveRunning) {
                ...
                if (restarting.compareAndSet(false, true)) {
                    try {
                        android.util.Log.w("OpenSquilla", "[保活] WebUI 连续失联，自动重启");
                        c.startWeb();
    ...
    private void refreshNotification() {
        if (c.getError() != null && !c.getError().isEmpty()) {
            updateNotification("OpenSquilla启动失败", c.getError());
        } else if (c.getMessage() != null && !c.getMessage().isEmpty()) {
            updateNotification("OpenSquilla运行中", "Web UI: http://127.0.0.1:" + c.getPort());
        }
    }
    ...
    private void createChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel ch = new NotificationChannel(
                    CHANNEL_ID, "OpenSquilla后台服务",
                    NotificationManager.IMPORTANCE_LOW);
            ch.setDescription("保持 OpenSquilla Web UI 后台运行");
